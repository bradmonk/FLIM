...
disp('Welcome to the FLIM dossier Toolbox')


%%

home = cd;
thisfile='FLIMdossierToolbox';
...
FLIMn.Names = {'ROI','sLife','sIntsy','sChi','sPix','dLife','dIntsy','dChi','dPix','Spine Count'};
FLIMs.ROI = FLIMd(:,1);
FLIMs.sLife = FLIMd(:,2);
...
% FLIMn.Names = {'ROI','sLife','sIntsy','sChi','sPix','dLife','dIntsy','dChi','dPix','Spine Count'};
Mus = [0, MUsLife, MUsIntsy, MUsChi, MUsPix, MUdLife, MUdIntsy, MUdChi, MUdPix, Nvals];

spn = sprintf('\n');
spf0 = sprintf(' set: % 7s ',fpdir);
spf00 = sprintf(' Total % 7s  = % 5.0f \n',FLIMn.Names{10},Mus(10));
spf1 = sprintf(' Mean Spine % 7s  = % 5.1f \n',FLIMn.Names{2}(2:end),Mus(2));
spf2 = sprintf(' Mean Spine % 7s  = % 5.1f \n',FLIMn.Names{3}(2:end),Mus(3));
spf3 = sprintf(' Mean Spine % 7s  = % 5.2f \n',FLIMn.Names{4}(2:end),Mus(4));
spf4 = sprintf(' Mean Spine % 7s  = % 5.1f \n',FLIMn.Names{5}(2:end),Mus(5));
spf5 = sprintf(' Mean Dendr % 7s  = % 5.1f \n',FLIMn.Names{6}(2:end),Mus(6));
spf6 = sprintf(' Mean Dendr % 7s  = % 5.1f \n',FLIMn.Names{7}(2:end),Mus(7));
spf7 = sprintf(' Mean Dendr % 7s  = % 5.2f \n',FLIMn.Names{8}(2:end),Mus(8));
spf8 = sprintf(' Mean Dendr % 7s  = % 5.1f \n',FLIMn.Names{9}(2:end),Mus(9));

disp([spn, spf0, spf00, spf1,spf2,spf3,spf4,spf5,spf6,spf7,spf8])
...
axes(hax1); hold(hax1, 'on')
    ...
    hax1.YLim(1) = 1900;
    hax1.YLim(2) = 2600;
    %hax1.YLim(1) = round(hax1.YLim(2) * 0.5);
    %hax1.YLim(2) = round(hax1.YLim(2) * 0.9);
...
str = {spf0,spn,spf00,spn,SPsL,SPdL,spn,SPsI,SPdI,spn,SPsC,SPdC,spn,SPsP,SPdP};
ft = annotation(fh1,'textbox', [.75 .45 .2 .52],'String', str,'FontSize',14);
